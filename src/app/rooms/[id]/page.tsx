...
import { getRooms, getReservations, createReservation, checkReservationConflicts, getRoomById, hasActiveReservation, checkMatriculaConflicts } from '@/lib/reservations'
...
export default function RoomDetailPage() {
  ...
  const [verificationStep, setVerificationStep] = useState<'select-time' | 'matricula' | 'code' | 'verified'>('select-time')
  const [verificationCode, setVerificationCode] = useState('')
  const [isSendingCode, setIsSendingCode] = useState(false)
  const [isVerifyingCode, setIsVerifyingCode] = useState(false)
  const [matriculaForVerification, setMatriculaForVerification] = useState('')
  ...
  // Resetear estado de verificación cuando se cierra el diálogo
  useEffect(() => {
    if (!isDialogOpen) {
      setVerificationStep('select-time')
      setVerificationCode('')
      setMatriculaForVerification('')
      setSelectedBlocks([])
    }
  }, [isDialogOpen])

  // Función para verificar conflicto de horario con reservas existentes
  const checkTimeConflict = async (matricula: string, selectedBlocks: number[]): Promise<boolean> => {
    if (selectedBlocks.length === 0) return false
    
    try {
      // Calcular las horas de inicio y fin basadas en los bloques seleccionados
      const hoy = new Date()
      const sorted = [...selectedBlocks].sort((a, b) => a - b)
      const horaInicio = sorted[0] + 7
      const horaFin = sorted[sorted.length - 1] + 8
      ...
      // Verificar conflictos usando checkMatriculaConflicts
      const conflicts = await checkMatriculaConflicts(matricula, inicio.toISOString(), fin.toISOString())
      return conflicts.length > 0
    } catch (error) {
      console.error('Error verificando conflictos de horario:', error)
      return false
    }
  }

  // Función para enviar código de verificación
  const handleSendVerificationCode = async (matricula: string) => {
    // Verificar si hay conflicto con el horario seleccionado ANTES de enviar el correo
    if (selectedBlocks.length === 0) {
      toast.error('Por favor selecciona un horario primero')
      return
    }
    
    try {
      const hasConflict = await checkTimeConflict(matricula, selectedBlocks)
      if (hasConflict) {
        toast.error('Ya tienes una reserva en este horario. Selecciona otro horario o cancela tu reserva anterior.')
        return
      }
    } catch (error) {
    ...
    try {
      ...
      setMatriculaForVerification(matricula)
      setVerificationStep('code')
      
      // Si hay un código dev (sin API key configurada), mostrarlo
      if (result.code) {
        toast.success('Código de verificación generado')
        toast.info(`Código: ${result.code}`, { duration: 15000 })
        console.log('🔐 Código de verificación:', result.code)
        if (result.warning) {
          console.warn('⚠️', result.warning)
        }
      } else {
        toast.success('Código de verificación enviado a tu correo institucional')
      }
    } catch (error) {
  ...
  const handleVerifyCode = async () => {
    ...
    try {
      const response = await fetch('/api/verification/verify', {
        ...
        body: JSON.stringify({
          matricula: matriculaForVerification,
  ...
  const onSubmit = async (data: ReservationForm) => {
    ...
    if (verificationStep !== 'verified' || data.matricula !== matriculaForVerification) {
      toast.error('Por favor verifica tu matrícula primero')
      setIsCreating(false)
      return
    }
    setIsCreating(true)
  ...
  const handleBlockClick = (blockIndex: number) => {
    ...
    setSelectedBlocks(prev => {
      ...
      if (prev.includes(blockIndex)) {
        ...
        // Si se deseleccionan todos los bloques, volver al paso de selección
      ...
      if (prev.length === 1) {
        ...
        if (Math.abs(blockIndex - firstBlock) === 1) {
          ...
          updateTimesFromBlocks(newSelection)
          // Avanzar al paso de matrícula cuando se seleccionen bloques
          if (verificationStep === 'select-time') {
            setVerificationStep('matricula')
          }
          return newSelection
        } else {
                ...
                <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                  ...
                  <DialogContent className="max-w-md">
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                      ...
                      {/* Paso 1: Seleccionar horario PRIMERO */}
                      {verificationStep === 'select-time' && (
                        ...
                        <p className="text-xs text-gray-500">
                          ...
                          <p className="text-xs text-amber-600 mt-2 font-medium">
                            ⚠️ Selecciona máximo 2 bloques consecutivos (2 horas) | No puedes tener dos reservas en el mismo horario
                          </p>
                        </div>
                          </div>
                        </div>
                      ...
                      {/* Paso 2: Ingresar matrícula y verificar conflicto */}
                      {verificationStep === 'matricula' && (
                        <div className="space-y-4">
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex gap-2">
                              <Input
                                ...
                                {...register('matricula')}
                                disabled={isSendingCode || isCreating}
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  ...
                                  if (!matricula) {
                                    toast.error('Por favor ingresa tu matrícula')
                                    return
                                  }
                                  if (selectedBlocks.length === 0) {
                                    toast.error('Por favor selecciona un horario primero')
                                  ...
                                  handleSendVerificationCode(matricula)
                                }}
                                disabled={isSendingCode || isCreating}
                                className="bg-[#FFCD00] hover:bg-[#E6B800] text-black"
                              >
                            ...
                            <p className="text-xs text-gray-500">
                              Se enviará un código de verificación a tu correo institucional (e0{watch('matricula') || 'XXXXX'}@cetys.edu.mx)
                            </p>
                          </div>
                        </div>
                      )}
                        ...
                        <div className="space-y-4">
                          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                            ...
                            <p className="text-xs text-blue-600 mt-1">
                              Revisa tu correo institucional: <strong>e0{matriculaForVerification}@cetys.edu.mx</strong>
                            </p>
                          </div>
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex gap-2">
                              <Input
                                ...
                                disabled={isVerifyingCode || isCreating}
                              ...
                              <Button
                                ...
                                disabled={isVerifyingCode || isCreating || verificationCode.length !== 6}
                          ...
                          {/* Campo oculto con la matrícula verificada */}
                          <input type="hidden" {...register('matricula')} value={matriculaForVerification} />
                        </div>
                      )}
                      
                      {/* Paso 4: Matrícula verificada, mostrar resumen */}
                      {verificationStep === 'verified' && (
                        <div className="space-y-4">
                          <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                            <p className="text-sm text-green-800 flex items-center gap-2 mb-2">
                              <span className="text-green-600">✓</span>
                              <strong>Matrícula verificada: {matriculaForVerification}</strong>
                            </p>
                            {selectedBlocks.length > 0 && (
                      ...
                      <div className="flex justify-end space-x-2 pt-4">
                        ...
                        <Button 
                          type="submit" 
                          disabled={isCreating || verificationStep !== 'verified'} 
                          className="bg-[#22C55E] hover:bg-[#16A34A] text-white"
                        >
